...
Dockerfileとは，イメージを作成するためのファイルである．基となるイメージをベースにして，コンテナに対して実行したい様々なコマンドを記述する．その後，Dockerfileに従って，イメージを作成する．Dockerfileからイメージを作成することを**ビルド**と言う．
...
補足だが，Dockerfileはイメージを作成する機能で，Docker Composeはコンテナとその周辺環境を作成・消去するものだ．また，YAMLファイルはスペースが意味をもちますので注意しましょう．

### 3.3 Kubernetesとは

Kubernetes (クーベネティス; k8s)とは，コンテナのオーケストレーションツールである．Dockerとは異なるツールになっている．オーケストレーションツールとは，システム全体を統括し，複数のコンテナを管理できるものだ．
